        ...
        stage('Preparation') {
            steps {
                script {
                    ...
                    currentBuild.displayName = "#${BUILD_NUMBER} - ${BUILD_TRIGGER_BY} - ${params.Tool_Id}"
                }
                sh 'rm -rf box/*'
                sh 'mkdir -p box/input box/logdir box/outdir'
                sh 'cp -L ./*.log box/input/'
            }
        }
        
        stage('Execution') {
            steps {
                sh 'python3 ./source/SOCAL_TrionRIEICP/FDC_Script_SOCAL_TrionRIEICP.py box/input/ ${Tool_Id} -outdir box/outdir/ -logdir box/logdir/ -ext log'
            }
        }
